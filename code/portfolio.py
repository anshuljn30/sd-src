class Portfolio:
    ...
    def calc_portfolio_return(self):
        ...
        self.return_usd = cdt.get_clean_data('ReturnUsd', dates=new_dates, ids=self.ids)
        self.return_local = cdt.get_clean_data('ReturnLocal', dates=new_dates, ids=self.ids)
        lagged_weights = self.weights.reindex(new_dates).shift(1).reindex(dates=new_dates)
